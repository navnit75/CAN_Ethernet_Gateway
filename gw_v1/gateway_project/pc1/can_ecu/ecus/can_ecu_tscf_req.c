...
uint16_t file_payld_idx = 0;

//static uint8_t send_buffer[2][8] = {{0x07, 0x10, 0xbb, 0xf1, 0x20, 0x01, 0x02, 0x03},
static uint8_t send_buffer[8];//[8] = {{0x07, 0x10, 0xbb, 0xf1, 0x20, 0x01, 0x02, 0x03},
...
void *recv_res_from_gw(void *args)
{
        ...
        response_time = (1000000*tv.tv_sec) + tv.tv_usec;
        fprintf(rrt_fp,"Round trip time in usecs = %ld\n",(response_time - request_time));
        fflush(rrt_fp);
        fprintf(stats_fp,"%d Round trip time in usecs = %ld\n", stats_count++, (response_time - request_time));
        fflush(stats_fp);
        request_time = 0;
        response_time = 0;
...
void *send_req_to_gw(void *args)
{
        ...
        while(1)
        {
                for(int i = 0; i < 8; i++)
                {
                        ...
                        {
                                send_buffer[i] = payload_buffer[send_payld_idx++];
                        }
                        else
                        {
                                send_buffer[i] = ' '; 
                ...
                do {
                        wr_ret = write(driver_fd, send_buffer, 8);
                        usleep(CAN_BUS_DELAY);
                }while(wr_ret == 0);
                ...
                if(once1)
                {
                        once1 = false;
                        gettimeofday(&tv,NULL);
                        request_time = (1000000*tv.tv_sec) + tv.tv_usec;
                }   
...
int main(int argc, const char *argv[])
{
        ...
        driver_fd = open(CAN_DEVICE_TSCF_req, O_RDWR);
