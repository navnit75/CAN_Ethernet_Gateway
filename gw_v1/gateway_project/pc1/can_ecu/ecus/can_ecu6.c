...
uint8_t send_buffer[2][8] = {{0x07, 0x10, 0xbb, 0xf1, 0x20, 0x01, 0x02, 0x03},
...
void *send_req_to_gw(void *args)
{
        ...
        while(1)
        {
                for(int i = 0; i < 3; i++)
                {
                        ...
                        {
                                send_buffer[0][5 + i] = payload_buffer[send_payld_idx++];
                        }
                        else
                        {
                                send_buffer[0][5 + i] = ' '; 
...
int main(int argc, const char *argv[])
{
        ...
        driver_fd = open(CAN_DEVICE5, O_RDWR);
...
starting:       file_payld_idx = 0;
                ...
                rrt_fp = fopen("/opt/gateway/can6_rt.txt", "w");
