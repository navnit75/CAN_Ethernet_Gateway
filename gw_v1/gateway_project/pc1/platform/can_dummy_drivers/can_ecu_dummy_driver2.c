...
extern gw_buff_t sh_buf2;
extern struct mutex can2_rdlock;
extern struct mutex can2_wrlock;
...
static int __init can_ecu_driver_init(void)
{
    ...
    err = alloc_chrdev_region(&dev, 0, MAX_DEV, "can_ecu_dummy_driver2");

    dev_major = MAJOR(dev);

    can_ecu_driver_class = class_create(THIS_MODULE, "can_ecu_dummy_driver2");
    ...
    for (i = 0; i < MAX_DEV; i++) {
        ...
        device_create(can_ecu_driver_class, NULL, MKDEV(dev_major, i), NULL, "can_ecu_dummy_driver2");
    }
    printk("can_ecu_dummy_driver2 Initialized \n");
...
static int can_ecu_driver_open(struct inode *inode, struct file *file)
{
    printk("can_ecu_dummy_driver2: open\n");
    return 0;
}

static int can_ecu_driver_release(struct inode *inode, struct file *file)
{
    printk("can_ecu_dummy_driver2: close\n");
    return 0;
}
...
static long can_ecu_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    printk("can_ecu_dummy_driver2: ioctl \n");
    return 0;
}
...
static ssize_t can_ecu_driver_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
   ...
   // printk("Reading from can_ecu_dummy_driver2\n"); //: %d\n", MINOR(file->f_path.dentry->d_inode->i_rdev));
...
static ssize_t can_ecu_driver_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
   ...
   // printk("Writing to can_ecu_dummy_driver2\n"); // %d\n", MINOR(file->f_path.dentry->d_inode->i_rdev));
