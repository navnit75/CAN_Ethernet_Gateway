...
uint8_t data[8];
struct mutex my_mutex1_ntsr;
struct mutex my_mutex2_ntsr;

EXPORT_SYMBOL(my_mutex1_ntsr);
EXPORT_SYMBOL(my_mutex2_ntsr);
...
char buff[8];
char buff1[8];
gw_buff_t sh_buf_ntr;

EXPORT_SYMBOL(sh_buf_ntr);
...
static int __init gw_ecu_driver_init(void)
{
        ...
        err = alloc_chrdev_region(&dev, 0, MAX_DEV, "gateway_ecu_dummy_driver_NTSCF_req");

        dev_major = MAJOR(dev);

        gw_ecu_driver_class = class_create(THIS_MODULE, "gateway_ecu_dummy_driver_NTSCF_req");
        ...
        for (i = 0; i < MAX_DEV; i++) {
                ...
                device_create(gw_ecu_driver_class, NULL, MKDEV(dev_major, i), NULL, "gateway_ecu_dummy_driver_NTSCF_req");
        }
        printk("gateway_ecu_dummy_driver_NTSCF_req Initialized\n");
...
static int gw_ecu_driver_open(struct inode *inode, struct file *file)
{
        ...
        mutex_init(&my_mutex1_ntsr);
        mutex_init(&my_mutex2_ntsr);
        printk("gateway_ecu_dummy_driver_NTSCF_req : open\n");
        return 0;
}

static int gw_ecu_driver_release(struct inode *inode, struct file *file)
{
        printk("gateway_ecu_dummy_driver_NTSCF_req : close\n");
        return 0;
}

static long gw_ecu_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
        printk("gateway_ecu_dummy_driver_NTSCF_req : ioctl\n");
        return (long) (gw_buff);
}
...
static ssize_t gw_ecu_driver_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
        ...
        if (gw_buff->cn2gw_pkt_arvl == 1)
        {
				...
				//printk("gw_ecu_driver read : CAN_DATA (%s)\n", gw_buff->cn2gw_buf);
        }
        else
        {
                count = 0;
