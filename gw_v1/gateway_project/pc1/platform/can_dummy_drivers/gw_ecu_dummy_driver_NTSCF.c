...
uint8_t data[8];
struct mutex my_mutex1_t;
struct mutex my_mutex2_t;

EXPORT_SYMBOL(my_mutex1_t);
EXPORT_SYMBOL(my_mutex2_t);
...
static int __init gw_ecu_driver_init(void)
{
        ...
        err = alloc_chrdev_region(&dev, 0, MAX_DEV, "gateway_ecu_dummy_driver_NTSCF");

        dev_major = MAJOR(dev);

        gw_ecu_driver_class = class_create(THIS_MODULE, "gateway_ecu_dummy_driver_NTSCF");
        ...
        for (i = 0; i < MAX_DEV; i++) {
                ...
                device_create(gw_ecu_driver_class, NULL, MKDEV(dev_major, i), NULL, "gateway_ecu_dummy_driver_NTSCF");
        }
        printk("gateway_ecu_dummy_driver_NTSCF Initialized\n");
...
static int gw_ecu_driver_open(struct inode *inode, struct file *file)
{
        ...
        mutex_init(&my_mutex1_t);
        mutex_init(&my_mutex2_t);
        printk("gateway_ecu_dummy_driver_NTSCF : open\n");
        return 0;
}

static int gw_ecu_driver_release(struct inode *inode, struct file *file)
{
        printk("gateway_ecu_dummy_driver_NTSCF : close\n");
        return 0;
}

static long gw_ecu_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
        printk("gateway_ecu_dummy_driver_NTSCF : ioctl\n");
        return (long) (gw_buff);
}
