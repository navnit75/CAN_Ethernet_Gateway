...
struct mutex can6_rdlock;
struct mutex can6_wrlock;

EXPORT_SYMBOL(sh_buf6);
EXPORT_SYMBOL(can6_rdlock);
EXPORT_SYMBOL(can6_wrlock);
gw_buff_t *gw_buff = &sh_buf6;
...
static int __init gw_ecu_driver_init(void)
{
        ...
        err = alloc_chrdev_region(&dev, 0, MAX_DEV, "gateway_ecu_dummy_driver6");

        dev_major = MAJOR(dev);

        gw_ecu_driver_class = class_create(THIS_MODULE, "gateway_ecu_dummy_driver6");
        ...
        for (i = 0; i < MAX_DEV; i++) {
                ...
                device_create(gw_ecu_driver_class, NULL, MKDEV(dev_major, i), NULL, "gateway_ecu_dummy_driver6");
        }
        gw_buff->cn2gw_pkt_arvl = 0;
        gw_buff->gw2cn_pkt_arvl = 0;
        printk("gateway_ecu_dummy_driver6 Initialized\n");
        return 0;
}
...
static int gw_ecu_driver_open(struct inode *inode, struct file *file)
{
        ...
        mutex_init(&can6_rdlock);
        mutex_init(&can6_wrlock);
        printk("gateway_ecu_dummy_driver6 : open\n");
        return 0;
}

static int gw_ecu_driver_release(struct inode *inode, struct file *file)
{
    printk("gateway_ecu_dummy_driver6 : close\n");
    return 0;
}

static long gw_ecu_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    printk("gateway_ecu_dummy_driver6 : ioctl\n");
    return (long) (gw_buff);
}
