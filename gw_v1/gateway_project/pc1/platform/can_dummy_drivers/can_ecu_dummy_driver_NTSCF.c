...
extern gw_buff_t sh_buf_n;
extern struct mutex my_mutex2_t;
extern struct mutex my_mutex1_t;

gw_buff_t *gw_buff = &sh_buf_n;
...
static int __init can_ecu_driver_init(void)
{
        ...
        err = alloc_chrdev_region(&dev, 0, MAX_DEV, "can_ecu_dummy_driver_NTSCF");

        dev_major = MAJOR(dev);

        can_ecu_driver_class = class_create(THIS_MODULE, "can_ecu_dummy_driver_NTSCF");
        ...
        for (i = 0; i < MAX_DEV; i++) {
                ...
                device_create(can_ecu_driver_class, NULL, MKDEV(dev_major, i), NULL, "can_ecu_dummy_driver_NTSCF");
        }
        printk("can_ecu_dummy_driver_NTSCF Initialized \n");
...
static int can_ecu_driver_open(struct inode *inode, struct file *file)
{
        printk("can_ecu_dummy_driver_NTSCF: open\n");
        return 0;
}

static int can_ecu_driver_release(struct inode *inode, struct file *file)
{
        printk("can_ecu_dummy_driver_NTSCF: close\n");
        return 0;
}
...
static long can_ecu_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
        ...
        printk("can_ecu_dummy_driver_NTSCF: ioctl \n");
        return 0;
}
...
static ssize_t can_ecu_driver_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
        ...
        //0 printk("Reading from can_ecu_dummy_driver_NTSCF\n"); //: %d\n", MINOR(file->f_path.dentry->d_inode->i_rdev));
        mutex_lock(&my_mutex1_t);
...
static ssize_t can_ecu_driver_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
        ...
        // printk("Writing to can_ecu_dummy_driver_NTSCF\n"); // %d\n", MINOR(file->f_path.dentry->d_inode->i_rdev));
        mutex_lock(&my_mutex2_t);
