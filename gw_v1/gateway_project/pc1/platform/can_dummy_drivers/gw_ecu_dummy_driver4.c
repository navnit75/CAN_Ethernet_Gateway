...
char buff1[8];
gw_buff_t sh_buf4;
struct mutex can4_rdlock;
struct mutex can4_wrlock;
EXPORT_SYMBOL(sh_buf4);
EXPORT_SYMBOL(can4_rdlock);
EXPORT_SYMBOL(can4_wrlock);
gw_buff_t *gw_buff = &sh_buf4;
...
static int __init gw_ecu_driver_init(void)
{
    ...
    err = alloc_chrdev_region(&dev, 0, MAX_DEV, "gateway_ecu_dummy_driver4");

    dev_major = MAJOR(dev);

    gw_ecu_driver_class = class_create(THIS_MODULE, "gateway_ecu_dummy_driver4");
    ...
    for (i = 0; i < MAX_DEV; i++) {
        ...
        device_create(gw_ecu_driver_class, NULL, MKDEV(dev_major, i), NULL, "gateway_ecu_dummy_driver4");
    }
        gw_buff->cn2gw_pkt_arvl = 0;
        gw_buff->gw2cn_pkt_arvl = 0;
    printk("gateway_ecu_dummy_driver4 Initialized\n");
...
static int gw_ecu_driver_open(struct inode *inode, struct file *file)
{
 ...
 mutex_init(&can4_rdlock);
    mutex_init(&can4_wrlock);
    printk("gateway_ecu_dummy_driver4 : open\n");
    return 0;
}

static int gw_ecu_driver_release(struct inode *inode, struct file *file)
{
    printk("gateway_ecu_dummy_driver4 : close\n");
    return 0;
}

static long gw_ecu_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    printk("gateway_ecu_dummy_driver4 : ioctl\n");
    return (long) (gw_buff);
}
