...
/*void read_from_conf_json_file(char *fname,char *buffer){
    ...
    /// parse the data read from file 
    parsed_json = json_tokener_parse(buffer_json);
    json_object_object_get_ex(parsed_json,"connection_type_critical",&connection_type);

    // copy into provided arguments 
    memcpy(buffer,json_object_get_string(connection_type),5);

    // print for verification 
    g_print(" Connection Type Critical : %s\n", buffer);
...
{
refresh_from_ecu_file_text("/opt/gateway/eth6.txt",ecu1_tview);
refresh_from_ecu_file_text("/opt/gateway/eth7.txt",ecu2_tview);
refresh_from_ecu_file_text("/opt/gateway/eth8.txt",ecu3_tview);
refresh_from_ecu_file_text("/opt/gateway/eth_ntscf.txt",ecu4_tview);

refresh_from_ecu_file("/opt/gateway/eth1_rt.txt",ecu1_rt_tview);
refresh_from_ecu_file("/opt/gateway/eth2_rt.txt",ecu2_rt_tview);
refresh_from_ecu_file("/opt/gateway/eth3_rt.txt",ecu3_rt_tview);
refresh_from_ecu_file("/opt/gateway/eth_tscf_rt.txt",ecu4_rt_tview);
...
{
    remove("/opt/gateway/eth6.txt");
    remove("/opt/gateway/eth7.txt");
    remove("/opt/gateway/eth8.txt");
    remove("/opt/gateway/eth_ntscf.txt");
    remove("/opt/gateway/eth1_rt.txt");
    remove("/opt/gateway/eth2_rt.txt");
    remove("/opt/gateway/eth3_rt.txt");
    remove("/opt/gateway/eth_tscf_rt.txt");
...
static void crecv_clicked(GtkWidget *button, gpointer data)
{
    ...
    FILE *fptr; 
    fptr = fopen("/opt/gateway/eth1_rt.txt","w");
    fclose(fptr);
    fptr = fopen("/opt/gateway/eth2_rt.txt","w");
    fclose(fptr);
    fptr = fopen("/opt/gateway/eth3_rt.txt","w");
    fclose(fptr);
    fptr = fopen("/opt/gateway/eth_tscf_rt.txt","w");
    fclose(fptr);
    refresh_from_recv_file();
}
...
int main(int argc, char *argv[])
{
    ...
    gtk_window_set_title(GTK_WINDOW(window), "ETH_ECU");
  ...
  //  gtk_container_add(GTK_CONTAINER(window), scrolledwindow);
    ...
    label = gtk_label_new("Send text(ETH-->CAN)");	
    gtk_fixed_put(GTK_FIXED(fixed), label, 10, 100);

    label = gtk_label_new("Received text(CAN_ECU)");	
    gtk_fixed_put(GTK_FIXED(fixed), label, 250, 100);

    label = gtk_label_new("Round trip time(usecs)[ETH-->GW/CAN/GW-->ETH]");	
